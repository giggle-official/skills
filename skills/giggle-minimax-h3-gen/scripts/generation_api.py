...
# -*- coding: utf-8 -*-
"""
giggle.pro MiniMax-H3 Generation API 封装脚本
...
SUPPORTED_DURATIONS = list(range(3, 16))   # 3–15 秒
...
class MiniMaxH3Client:
    ...
    def _validate(self, model: str, duration: int, aspect_ratio: str,
                  resolution: str, generating_count: int = 1) -> None:
        if model not in SUPPORTED_MODELS:
            raise ValueError(f"不支持的模型 {model!r}，可用: {', '.join(SUPPORTED_MODELS)}")
        if duration not in SUPPORTED_DURATIONS:
            raise ValueError(f"时长必须为 3–15 秒，当前: {duration}")
        if aspect_ratio not in SUPPORTED_RATIOS:
            raise ValueError(
                f"不支持的画面比例 {aspect_ratio!r}，可用: {', '.join(SUPPORTED_RATIOS)}"
            )
        if resolution not in SUPPORTED_RESOLUTIONS:
            raise ValueError(
                f"不支持的分辨率 {resolution!r}，可用: {', '.join(SUPPORTED_RESOLUTIONS)}"
            )
    ...
    def text_to_video(self, *, prompt: str, model: str, duration: int,
        ...
        self._validate(model, duration, aspect_ratio, resolution, generating_count)
...
def handle_query(client: MiniMaxH3Client, task_id: str) -> None:
    """单次查询任务状态"""
...
def handle_wait(client: MiniMaxH3Client, task_id: str,
...
def handle_submit(client: MiniMaxH3Client, args) -> None:
    mode  = args.mode
    model = args.model
    dur   = args.duration
    ...
    # 提交后立即开始轮询，完成后主动推送结果
    print("\n正在用 MiniMax-H3 生成，完成后我会主动通知你（约 1–5 分钟）...",
          file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="giggle.pro MiniMax-H3 视频生成",
...
示例:
  ...
  # 文生视频
  python3 generation_api.py --mode text --prompt "镜头缓缓推进，橘猫在阳光下玩耍" --duration 4

  # 图生视频（首帧 URL）
  python3 generation_api.py --mode image --prompt "人物缓缓转身" \\
    --start-frame "url:https://example.com/photo.jpg" --duration 4
...
def main() -> None:
    ...
    client = MiniMaxH3Client(api_key)
