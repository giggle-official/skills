...
const MODEL = 'openclaw/discord-community-growth-expert';
const PROFILE_PATH = '/home/storyclaw/.openclaw/workspace-discord-community-growth-expert/outputs/user_profile.json';
const DASHBOARD_CONFIG_PATH = '/home/storyclaw/.openclaw/workspace-discord-community-growth-expert/outputs/dashboard_config.json';
const DB_PATH = '/home/storyclaw/.claw/shared/shared.db';
...
function updateDashboard(parsed, username) {
  ...
  if (!cfg || !cfg.enabled || !cfg.widget_ids) return;
  ...
  try {
    ...
    if (intentIdx >= 0 && cfg.widget_ids.intent_distribution) {
      const row = getWidget.get(cfg.widget_ids.intent_distribution);
      if (row) {
        const data = JSON.parse(row.data);
        data[intentIdx] = (data[intentIdx] || 0) + 1;
        setWidget.run(JSON.stringify(data), cfg.widget_ids.intent_distribution);
      }
    }

    // High-intent users table
    if (parsed.highIntent && cfg.widget_ids.high_intent_users) {
      const row = getWidget.get(cfg.widget_ids.high_intent_users);
      if (row) {
        ...
        setWidget.run(JSON.stringify(data), cfg.widget_ids.high_intent_users);
      }
    }

    // Feedback issues table
    if (parsed.feedback && !parsed.feedback.startsWith('暂无') && cfg.widget_ids.feedback_issues) {
      const row = getWidget.get(cfg.widget_ids.feedback_issues);
      if (row) {
        let data = JSON.parse(row.data);
        ...
        if (data.length > 20) data = data.slice(0, 20);
        setWidget.run(JSON.stringify(data), cfg.widget_ids.feedback_issues);
      }
    }

    // Key metrics — increment total messages and high-intent count
    if (cfg.widget_ids.key_metrics) {
      const row = getWidget.get(cfg.widget_ids.key_metrics);
      if (row) {
        const data = JSON.parse(row.data);
        ...
        setWidget.run(JSON.stringify(data), cfg.widget_ids.key_metrics);
      }
    }
...
### 推荐回复
[直接可发送给用户的消息，结构：先回应问题 → 补充有用信息 → 给出轻行动]
...
[高频问题 / 内容空白 / 高意向线索 / 策略信号，如无则写"暂无"]`;
  ...
  try {
    ...
    if (parsed.reply) {
      ...
      // Fallback: if parsing fails, send the raw output but warn
      console.warn('⚠️ Could not parse 推荐回复 section, sending raw output');
      const trimmed = fullOutput.length > 2000 ? fullOutput.slice(0, 1997) + '...' : fullOutput;
      await message.reply(trimmed);
    }
