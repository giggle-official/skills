...
本文档说明如何正确配置 Twitter Automator 的定时任务，避免会话 token 耗尽问题。
...
```json
{
  "sessionTarget": "session:agent:twitter-automated-operation:main",
  "payload": {
    "kind": "agentTurn",
    "message": "执行任务..."
  }
...
**作用**: 从数据池选取 Top 3，生成视频并发布到 Twitter

```bash
openclaw cron add --job '{
  "name": "Twitter内容生产（每天 10:00）",
  "schedule": {
    "kind": "cron",
    "expr": "0 10 * * *",
    "tz": "Asia/Shanghai"
