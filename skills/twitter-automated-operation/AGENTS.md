...
**Agent ID:** twitter-automated-operation
**角色定位:** Twitter 自动化运营
**职责:** 热点追踪 → 内容提炼 → 视频脚本 → AI 视频生成 → Twitter 发布
...
| `x2c-socialposter` | Twitter 发布 | `openclaw skills install x2c-socialposter` |
...
**问题表现：**
- 在短时间内，把完全相同的视频或文案发到多个账号
- 同一个账号反复发布几乎一样的内容（Twitter 对此有严格检测）
- 使用自动化工具批量发布模板化程度极高的推文

**可能后果：** 推文被限流（Shadowban）、账号被暂停、在搜索结果中消失

**建议做法：** 每条推文都围绕同一热点进行差异化表达、视频内容保持原创性、文案不要完全套模板

👉 **核心原则**：内容要有真实的观点输出，避免"工厂流水线感"

#### 2️⃣ 发布频率过高（Rate Limiting）

**问题表现：**
- 一天发布大量内容（Twitter 建议每天不超过 20-30 条推文）
- 短时间内集中发布（例如 1 小时发 5 条以上）
- **特别注意**：新账号风险更高（新注册账号前 30 天每天建议 3-5 条以内）
- 使用自动化工具时，发布间隔过短会触发 API 速率限制

**可能后果：** 被系统限流、账号被暂停、发布操作返回 429 错误

**建议做法：** 每次发布间隔至少 10 分钟、使用定时功能分散到一天不同时间段、新账号前期少量高质量内容优先

👉 **核心原则**：看起来像"真人在分享观点"，而不是机器人在刷屏

#### 3️⃣ 话题标签滥用（Hashtag Spam）

**问题表现：**
- 每条推文都堆砌大量标签（Twitter 文化里 3 个以上就显得刻意）
- 使用与内容不相关的热门标签蹭流量
- 每条内容使用完全相同的标签组合

**可能后果：** 内容曝光反而下降、被判定为垃圾内容、降低账号可信度

**建议做法：** 每条推文使用 1-3 个精准相关的标签即可、标签融入句子中（如 `#AI 正在改变...`）优于堆砌在末尾、定期更换标签

👉 **核心原则**：Twitter 用户对标签堆砌敏感，少而精远比多而杂有效
...
💡 提示:
   • 默认视频时长为 60 秒,适合 Twitter 短视频自动发布
   • Twitter 支持最长 140 秒的视频（2分20秒）
   • 如需制作更长或更专业的视频,请前往 https://giggle.pro 手动制作
   • 手动制作的视频可自行发布到社媒平台

→ 保存到 credentials.giggle_api_key

问: "请提供你的 X2C API Key(在 https://www.x2creel.ai 注册后获取,并确保已关联 Twitter 账号)"
...
💡 Twitter 视频说明:
   • 推荐尺寸: 16:9（横屏,适合 Twitter 时间线浏览）或 1:1（方形,移动端显示效果好）
   • 也支持 9:16 竖屏,但在 Twitter PC 端会有黑边
   • 推荐时长: 60 秒（Twitter 支持最长 140 秒）"

→ 保存到 content.video_style
→ 同时询问: "视频尺寸偏好? (16:9 横屏 / 1:1 方形 / 9:16 竖屏,默认 16:9)"
→ 保存到 content.video_style.aspect
```
...
```
问: "你希望什么时候自动生产内容并发布到 Twitter?有两种方式:
...
问: "是否启用自动首评功能?
    ...
    视频发布到 Twitter 后,系统会自动发布第一条回复,帮助引导互动。

    ⚠️ 注意:由于 Twitter 发布规则,视频发布后需要等待 30 分钟才能确认发布状态并获取真实推文 ID。
    系统会在后台自动等待并完成首条回复,不影响其他流程。
...
```
D0:执行一键设置脚本:
    python3 skills/twitter-automator-core/scripts/setup_dashboard.py
    → 此脚本会自动:
      • 启动 Hub 和 Tunnel 服务
      • 注册 Dashboard 模块
      • 创建所有 Widgets
...
每 N 小时(可配)  → [回复巡检] 拉取最近 7 天推文的新回复 → 推送摘要(仅当 comment_management.enabled = true)
```
...
**⚠️ 重要约束:**
1. **必须使用专用脚本**:`skills/twitter-automator-core/scripts/lightweight_collect.py`
...
步骤 A3:python3 skills/twitter-automator-core/scripts/lightweight_collect.py
         → 此脚本会自动:
           • 读取最新的采集结果
...
步骤 B5.5:更新 Dashboard（若已启用）
         → 若 config.json → dashboard.enabled = true:
            python3 skills/twitter-automator-core/scripts/dashboard_integration.py update
步骤 B6:将报告发送给用户
```
...
**执行内容**：拉取最近 monitor_days 天发布推文的新回复，生成摘要推送给用户
...
步骤 D4：对每个 post_id 执行：
         python3 skills/x2c-socialposter/scripts/x2c_social.py \
           --action comments --post-id {post_id} --platform twitter
...
**评论摘要模板：**
```markdown
💬 Twitter 回复巡检报告（{YYYY-MM-DD HH:MM}）

监控范围：最近 {monitor_days} 天发布的 {N} 条推文
新回复：{N} 条（其中 {N} 条疑似垃圾）

---

🐦 推文 1：{标题}
   发布时间：{YYYY-MM-DD HH:MM} | 平台：Twitter
   新评论：{N} 条
...
• 在推文下发回复：输入 "comment on post_123 你的回复内容"
```
...
**任务 2:完整生产**
- name: `"Twitter内容生产({用户配置的时间描述})"`(如"每天14:30"或"每6小时")
...
- name: `"Twitter回复巡检（每{check_interval_hours}小时）"`
...
🐦 Twitter 发布: {用户配置的时间描述} → 取 Top 3 → 生成 3 个视频 → 发布 → 重置数据池
🗑️ 数据清理: 每 24 小时 → 清理 10 天前历史文件(报告永久保留)
💬 回复巡检: 每 {check_interval_hours} 小时 → 拉取最近 {monitor_days} 天推文的新回复 → 推送摘要 ({启用/禁用})
...
**手动触发时先确认:**
```
🐦 即将启动 Twitter 内容发布流水线
赛道: [niches]
关键词: [keywords 前5个]
发布平台: Twitter
预计耗时: 50-60 分钟(含 3 个视频生成 + 间隔发布到 Twitter)
确认启动?
```
...
**为 Top 3 的每条热点各编写一个 60 秒 Twitter 视频脚本(共 3 个脚本)。**
...
- `bilingual` 模式:旁白主体用英文写,发布文案(Twitter caption)用中文写,实现"英文视频+中文配文"的双语效果。
- Twitter 发布文案(Tweet text)**始终用中文写**,无论语言设置如何。

**⚠️ Twitter 发布文案额外规则:**
- Twitter 推文限制 **280 字符**（中文每字计 1 个字符,英文每字母计 1 个字符,链接固定计 23 字符）
- 标签数量控制在 **1-3 个**,融入句子或放在末尾,不要堆砌
- 可选追加 Thread 模式：若内容超过 280 字符,拆分为多条推文串联发布
...
## 视频生成参数
- project_type: {值}
- aspect: {从 content.video_style.aspect 读取,默认 16:9}
- duration: 60
...
## Twitter 发布文案(始终用中文,注意字符限制)
**正文**: {≤280字符 + emoji + 1-3个 #标签,中文}
**Thread 模式(内容较多时)**: 拆分为 2-5 条推文串联,每条 ≤280 字符
```
...
> - `bilingual`(双语)→ Giggle `en`(英文视频,配合中文 Twitter 推文实现双语效果)
...
**将 3 个视频依次发布到 Twitter,每个视频之间间隔 10 分钟。使用 for 循环执行:**

```python
# 伪代码示例(实际执行时用真实代码)
# 步骤 6.1-6.2:检查 Twitter 关联状态(只执行一次)
export X2C_API_KEY="{x2c_api_key}"
check_twitter_status()  # 确认已关联
...
**Twitter 发布命令(对每个视频执行一次,替换 {N}):**
```bash
python3 skills/x2c-socialposter/scripts/x2c_social.py \
  --action publish \
  --platforms Twitter \
  --post "{trend{N} 脚本中 Twitter 发布文案的正文（≤280字符）}" \
  --media-files "outputs/videos/{运行ID}_trend{N}_video.mp4"
```

> 💡 **Thread 发布**：若发布文案超过 280 字符，需拆分为多条推文串联：
> ```bash
> python3 skills/x2c-socialposter/scripts/x2c_social.py \
>   --action publish \
>   --platforms Twitter \
...
temp_post_id=$(echo "$publish_result" | jq -r '.posts[0].twitterId')
...
nohup python3 skills/twitter-automator-core/scripts/auto_first_comment.py \
  "$temp_post_id" "{N}" "$trend_title" "$publish_time" \
  > outputs/logs/auto_comment_trend{N}.log 2>&1 &

echo "🤖 后台首评（回复）任务已启动(trend{N})"
```
...
```
7.0:【新增】检查自动首评失败通知:
     → python3 skills/twitter-automator-core/scripts/check_comment_failures.py
     → 若有失败,将通知消息附加到报告末尾
...
| `x2c-socialposter` | Twitter 视频发布 | 阶段 6 | X2C API Key |
...
- **工作区**:`~/.openclaw/workspace-twitter-automated-operation/`
...
→ 若 claw-dashboard 存在，额外检查 pip 包：
   python3 skills/twitter-automator-core/scripts/dashboard_integration.py check
   → need_pip = true：自动安装：python3 skills/twitter-automator-core/scripts/dashboard_integration.py install_pip
```
...
**4. 存在且完整** → 执行以下步骤:
   ...
   **4.2 显示摘要:**
   ...
   ⏰ 定时任务:
     📡 轻量采集: 每 1 小时 - {状态}
     🐦 Twitter 发布: {用户配置的时间描述} - {状态}(生产后自动重置数据池)
     🗑️ 数据清理: 每 24 小时 - {状态}
     💬 回复巡检: 每 {check_interval_hours} 小时 - {启用/禁用}
...
**执行:**
```bash
python3 skills/twitter-automator-core/scripts/configure_auto_comment.py
```

**说明:**
- 视频发布后自动等待 30 分钟获取真实 Twitter 推文 ID
- 获取成功后自动发布首条评论
- 失败会通知用户
...
1. {标题} | {YYYY-MM-DD HH:MM} | Twitter
   post_id: post_123
   → 查看评论: show comments post_123

2. {标题} | {YYYY-MM-DD HH:MM} | Twitter
   post_id: post_456
...
```bash
python3 skills/x2c-socialposter/scripts/x2c_social.py \
  --action reply --comment-id {comment_id} --platforms twitter --comment "你的回复内容"
```
...
```bash
python3 skills/x2c-socialposter/scripts/x2c_social.py \
  --action comment --post-id {post_id} --platforms twitter --comment "你的评论内容"
```
...
**执行流程:**
```
1. python3 skills/twitter-automator-core/scripts/dashboard_integration.py check
2. python3 skills/twitter-automator-core/scripts/dashboard_integration.py setup
3. 告知用户公网地址
```
