...
PRESIGN_URL="https://giggle.pro/api/v1/asset/get-presigned-url"
REGISTER_URL="https://giggle.pro/api/v1/asset/register"

# --- Resolve API key (same as other giggle skills) ---
API_KEY="${GIGGLE_API_KEY:-}"
if [[ -z "$API_KEY" ]]; then
  echo '{"error":"No API key. Set GIGGLE_API_KEY."}' >&2
  exit 1
fi
...
PRESIGN_RESP="$(curl -sS -X POST "$PRESIGN_URL" \
...
REGISTER_RESP="$(curl -sS -X POST "$REGISTER_URL" \
  -H "Content-Type: application/json" \
  -H "Accept: */*" \
  -H "x-auth: $API_KEY" \
...
# --- Output result (API returns: asset_id, name, type, file_url) ---
echo "$REGISTER_RESP" | jq '{
  asset_id: .data.asset_id,
  name: .data.name,
  type: .data.type,
  file_url: .data.file_url,
  public_url: (.data.file_url // .data.public_url),
