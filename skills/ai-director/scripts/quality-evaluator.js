...
async function callGemini(videoUrl, prompt) {
  if (!GEMINI_API_KEY) {
    throw new Error('Gemini API key not configured. Set geminiApiKey in config.json or GEMINI_API_KEY env var.');
  }
...
async function evaluateVideo(videoUrl, originalPrompt = "") {
  ...
  if (!result.total_score) {
    result.total_score = Math.round((result.single_clip.weighted_score + result.assembly.weighted_score) / 2);
  }
...
function printReport(result) {
  ...
  for (const [key, value] of Object.entries(result.single_clip)) {
    ...
    if (typeof value === 'object' && value.score !== undefined) {
      const bar = '█'.repeat(Math.round(value.score / 10)) + '░'.repeat(10 - Math.round(value.score / 10));
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.log('  node quality-evaluator.js https://example.com/video.mp4 --prompt "校园恋爱短剧"');
    console.log('');
