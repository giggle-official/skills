...
- [ ] `skills/instagram-automator-core/scripts/lightweight_collect.py` - 轻量采集脚本
- [ ] `skills/instagram-automator-core/scripts/setup_dashboard.py` - Dashboard 设置脚本
- [ ] `skills/instagram-automator-core/scripts/dashboard_integration.py` - Dashboard 集成脚本
...
# 2. 打包
tar -czf instagram-automator-$(date +%Y%m%d).tar.gz \
...
# 3. 验证打包
tar -tzf instagram-automator-$(date +%Y%m%d).tar.gz | head -20
```
