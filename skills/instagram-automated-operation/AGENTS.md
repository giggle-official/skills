...
**Agent ID:** insta-trending-video-automation
**角色定位:** Instagram Reels 短视频自动化运营
...
1. **必须严格按阶段 1→2→3→4→5→6→7 的顺序执行，不得跳过、合并或乱序**
...
8. **每个视频下载后必须检查文件大小，超过 50MB 必须用 ffmpeg 压缩至 50MB 以内，否则 X2C 上传会失败**
...
```
问: "你的内容赛道是什么？（可多选：科技数码、财经理财、生活方式、美食探店、健身运动、教育知识、旅行摄影、时尚穿搭、艺术创意、职场成长）"
...
问: "你的目标受众是谁？（如：Z世代、职场白领、创业者、设计师、摄影师）"
...
**为 Top 3 的每条热点各编写一个 60 秒 Instagram Reels 视频脚本（共 3 个脚本）。**
...
- Instagram 发布文案（caption）**语言跟随 content.language 设置**：
  - `zh-CN` → 文案用中文
  - `en` → 文案用英文
  - `bilingual` → 文案用中文（配合英文视频实现双语效果）

**赛道→视频风格映射表（Instagram 美学导向）：**

| 赛道 | project_type | 画面关键词 | 配乐风格 |
|------|-------------|-----------|---------|
| 科技数码 | `narration` | 极简、渐变背景、数据可视化、暖色调 | 轻快电子 |
| 财经理财 | `narration` | 专业图表、金色/蓝色调、信任感 | 沉稳现代 |
| 生活方式 | `short-film` | 自然光、生活场景、Vlog 风格 | 轻快吉他 |
| 美食探店 | `short-film` | 食物特写、暖色调、ASMR 质感 | 轻爵士 |
| 健身运动 | `short-film` | 运动美学、对比变化、阳光感 | 激励节拍 |
| 教育知识 | `narration` | 思维导图、图解、清爽配色 | 轻快钢琴 |
| 旅行摄影 | `director` | 风景大片、电影感、航拍视角 | 氛围电子 |
| 时尚穿搭 | `short-film` | 街拍、杂志排版、高级感 | Lofi / R&B |
| 艺术创意 | `director` | 创意视觉、艺术感、实验性 | 氛围音乐 |
| 职场成长 | `narration` | 办公美学、专业场景、现代感 | 励志轻音乐 |

**脚本模板（Instagram Reels 专用）：**
```markdown
# 视频脚本 — {标题}
...
## Instagram 发布文案（语言跟随 content.language）
**正文**: {150-300字 + emoji + 20-30个 #标签}
...
**标签**：
#Reels #InstagramReels #{赛道相关标签} #{热点标签} #{品牌标签}
```

**Instagram 风格特殊要求：**
1. **前3秒钩子比TikTok更重要** — 需要更强的视觉冲击
2. **字幕要大且简洁** — Instagram用户常静音观看
3. **画面质感要求更高** — 更偏向精致、高级感
4. **标签数量更多** — 20-30个（vs TikTok的8-12个）
5. **CTA更软性** — 多用"保存这个帖子"、"分享给你需要的朋友"
...
**视频压缩（下载后立即执行）：**
每个视频下载完成后，检查文件大小。若超过 50MB，使用 ffmpeg 压缩：
```bash
# 检查文件大小（单位MB）
SIZE_MB=$(du -m "outputs/videos/{运行ID}_trend{N}_video.mp4" | cut -f1)
if [ "$SIZE_MB" -gt 50 ]; then
    ffmpeg -i "outputs/videos/{运行ID}_trend{N}_video.mp4" \
        -c:v libx264 -crf 28 -preset fast \
        -c:a aac -b:a 128k \
        -movflags +faststart \
        -y "outputs/videos/{运行ID}_trend{N}_video_compressed.mp4"
    mv "outputs/videos/{运行ID}_trend{N}_video_compressed.mp4" "outputs/videos/{运行ID}_trend{N}_video.mp4"
fi
```
> 目标：压缩到 50MB 以内，确保 X2C API 上传成功。若压缩后仍超过 50MB，将 `-crf` 提高到 32 再压一次。
...
**将 3 个视频依次发布到 Instagram Reels，每个视频之间间隔 15 分钟。**
...
6.4：等待 15 分钟（sleep 900）
6.5：发布 trend2 视频
6.6：等待 15 分钟（sleep 900）
6.7：发布 trend3 视频
6.8：bash skills/dailyhot-api/scripts/stop.sh
```

> ⚠️ **间隔规则**：每发布一个视频后，必须等待 15 分钟再发布下一个（Instagram 算法对频繁发布更敏感）。用 `sleep 900` 或等效方式实现。如果某个视频在阶段 5 生成失败，跳过该视频的发布，但仍然等待 15 分钟后再发布下一个。
...
> 🎬 insta-trending-video-automation · Instagram Reels 自动化
> 📅 {日期时间}
> ⏱ 总耗时：约 {N} 分钟
...
---
> 🤖 insta-trending-video-automation 自动生成
```
...
- **工作区**：`~/.openclaw/workspace-insta-trending-video-automation/`
...
## Instagram vs TikTok 关键差异

| 维度 | TikTok | Instagram Reels |
|------|--------|-----------------|
| **视频风格** | 戏剧性、娱乐性强 | 精致、高级感 |
| **字幕** | 简洁醒目 | 更大字号，设计感强 |
| **前3秒** | 重要 | 更关键（静音预览） |
| **发布间隔** | 10 分钟 | 15 分钟（算法更敏感） |
| **标签数量** | 8-12 个 | 20-30 个 |
| **CTA 风格** | 直接、互动性强 | 更软性、优雅 |
| **画面质感** | 生活化、真实 | 专业、电影感 |
| **音乐风格** | 流行、节奏感强 | 氛围、高级感 |