...
- `bilingual` 模式：旁白主体用英文写，发布标题和描述（Youtube title + description）用中文写，实现"英文视频+中文配文"的双语效果。
- Youtube 发布信息（标题和描述）**始终用中文写**，无论语言设置如何。
...
## Youtube 发布信息（始终用中文）
**标题**: {15-50字，中文，简洁有力，带核心悬念或数据亮点，禁止超过100字符}
**描述**: {150-300字 + emoji + 8-12个 #标签，中文，展开说明 + 引导互动}
```

> ⚠️ **标题与描述必须分开编写，不得相同。**
> - **标题**：用于 Youtube 视频标题（`--title` 参数），限制 100 字符以内，目标 15-50 个中文字。要求：简洁有力、带悬念或数据亮点、能引发点击欲。
> - **描述**：用于 Youtube 视频描述（`--post` 参数），150-300 字，包含事件展开、观点、提问互动、emoji 和 hashtag。
...
**将 3 个视频依次发布到 Youtube，每个视频之间间隔 10 分钟。**
...
python3 skills/x2c-socialposter/scripts/x2c_social.py \
  --action publish \
  --platforms youtube \
  --title "{trend{N} 脚本中 Youtube 发布信息的标题，≤100字符}" \
  --post "{trend{N} 脚本中 Youtube 发布信息的描述}" \
  --media-files "outputs/videos/{运行ID}_trend{N}_video.mp4"
```

> ⚠️ **`--title` 和 `--post` 必须使用脚本中分别编写的标题和描述，不得用同一段文案。`--platforms` 必须用小写 `youtube`。**
...
**规则**：报告自包含，提炼卡、标题和描述完整嵌入。生成后必须发送给用户。
