...
本文档说明如何正确配置 TikTok Automator 的定时任务，避免会话 token 耗尽问题。
...
```json
{
  "sessionTarget": "session:agent:tiktok-automator-cn-2026:main",
  "payload": {
    "kind": "agentTurn",
    "message": "执行任务..."
  }
...
**作用**: 从数据池选取 Top 3，生成视频并发布到 TikTok

```bash
openclaw cron add --job '{
  "name": "Tiktok内容生产（每天 10:00）",
  "schedule": {
    "kind": "cron",
    "expr": "0 10 * * *",
    "tz": "Asia/Shanghai"
