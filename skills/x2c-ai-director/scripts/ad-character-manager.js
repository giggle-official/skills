...
async function listCharacters(apiKey) {
  ...
  if (result.characters && result.characters.length > 0) {
    console.log(`Found ${result.characters.length} character(s) (max allowed: ${result.max_allowed})`);
    console.log('');
...
async function createCharacter(apiKey, name, gender, imageSource, imageType = 'image/png') {
  ...
  if (imageSource.startsWith('/9j/') || imageSource.startsWith('data:') || imageSource.length > 200) {
    // Base64 mode
...
async function deleteCharacter(apiKey, characterId) {
  ...
  const result = await request('character/delete', {
    ...
    switch (command) {
      ...
      case 'create': {
        ...
        if (!name || !gender) {
          ...
          console.log('   node ad-character-manager.js create <name> <gender> --base64 <base64_data> [--type image/jpeg]');
          console.log('');
          console.log('Example:');
          console.log('  node ad-character-manager.js create Alice female https://example.com/alice.png');
          console.log('  node ad-character-manager.js create Bob male --base64 "/9j/4AAQ..." --type image/jpeg');
          process.exit(1);
        }
        ...
        await createCharacter(apiKey, name, gender.toLowerCase(), options.base64 || options.url, imageType);
        break;
      }
