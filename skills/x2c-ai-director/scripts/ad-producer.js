...
const commands = {
  ...
  async 'produce-video'(projectId, episode, options = {}) {
    ...
    const res = await request('video/produce', {
    ...
    if (options.wait) {
      ...
      const result = await poll(async () => {
        const status = await request('video/query', { 
          project_id: projectId, 
          episode_number: episodeNum 
        }, apiKey);
      ...
      }, { maxAttempts: 360, intervalMs: 10000 }); // Up to 60 minutes
  ...
  async 'video-status'(projectId, episode) {
    ...
    const res = await request('video/query', {
      project_id: projectId,
      episode_number: episodeNum
    }, apiKey);
    ...
    if (res.video_asset) {
      ...
      if (res.video_asset.thumbnail_url) console.log(`   Thumbnail: ${res.video_asset.thumbnail_url}`);
    }
...
module.exports = {
  ...
  produceVideo: async (projectId, episodeNumber = 1) => {
    const apiKey = accountManager.getApiKey();
    if (!apiKey) throw new Error('X2C account not bound');
    return request('video/produce', { project_id: projectId, episode_number: episodeNumber }, apiKey);
  },
