...
def collect_data():
    ...
    for p in publish_logs:
        try:
            ...
            pass
    data["total_posts"] = total_published
    ...
    for log_file in sorted(publish_logs, reverse=True):
        try:
            ...
            for idx, t in enumerate(plog.get("trends", []), start=1):
                script_file = f"outputs/scripts/{run_id}_trend{idx}_post.md" if run_id else ""
                caption = _extract_caption(script_file)
                post_id = t.get("linkedin_post_id", t.get("post_id", ""))
                recent_posts.append({
                    "标题": t.get("title", ""),
                    "发布时间": t.get("publish_time", ""),
                    "状态": "✅" if t.get("status") == "success" else "❌",
                    "PostID": post_id,
                    "LinkedIn链接": t.get("post_url", "-"),
                    "发布内容": caption,
                    "图片文件": t.get("image_file", "")
                })
        except Exception:
            continue
    # 按发布时间倒序排列（最新的在前）
    ...
    data["recent_posts"] = recent_posts or [{
        "标题": "暂无数据",
        "发布时间": "-",
        "状态": "-",
        "PostID": "-",
    ...
    # 运营统计
    data["stats"] = [
        {"label": "平均耗时", "value": "35分钟" if recent_posts else "--"},
        {"label": "成功率", "value": "100%" if recent_posts else "--"},
...
def update():
    ...
    upd("total_videos", [data["total_posts"]])
    ...
    upd("recent_videos", data["recent_posts"])
    ...
    return {
        ...
        "data": {
            ...
            "recent_post_count": len(data["recent_posts"]),
            "recent_posts_fields": list(data["recent_posts"][0].keys()) if data["recent_posts"] else []
        }
    }
