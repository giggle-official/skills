...
4.5：按模板生成“图片生成提示词 + LinkedIn 帖子文案”（禁止编造，必须来自提炼卡内容）
4.6：写入 outputs/scripts/{运行ID}_trend{N}_image_prompt.md
```
...
{结构建议：开头一句引发共鸣/观点；主体 80-160 字展开（不得超出提炼卡）；结尾 1 个提问引导互动。}
（语言：按 4.4 与 语言规则生成；结尾附 8-12 个 #标签，emoji 适量 1-3 个。）
```
