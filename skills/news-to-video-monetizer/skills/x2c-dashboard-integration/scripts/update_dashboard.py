...
使用 workspace 内置的 x2c-real-dashboard skill
"""
...
# 使用 workspace 内置的 x2c-real-dashboard
WORKSPACE_SKILLS = os.path.expanduser('~/.openclaw/workspace-news-to-video-monetizer/skills')
X2C_DASHBOARD_SCRIPTS = os.path.join(WORKSPACE_SKILLS, 'x2c-real-dashboard/scripts')
...
def get_x2c_data(api_key, script_name, *args):
    ...
    script_path = os.path.join(X2C_DASHBOARD_SCRIPTS, script_name)
    
    if not os.path.exists(script_path):
        print(f"❌ 脚本未找到: {script_path}", file=sys.stderr)
        return None
...
def update_dashboard(api_key):
    ...
    for i, t in enumerate(trend_data):
        if i % 5 == 0 or i == len(trend_data) - 1:
            labels_30d.append(t['date'][5:])
        else:
            labels_30d.append('')
    ...
    for item in items:
        ...
        for platform in ['tiktok', 'youtube', 'instagram', 'twitter', 'facebook']:
            views = pv.get(platform, 0)
            if views > 0:
                name = platform_names.get(platform, platform.capitalize())
                platforms_display.append(f"{name} {views:,}")
        
        # 用 | 分隔各平台
        platforms_str = " | ".join(platforms_display) if platforms_display else "-"
    ...
    print(f"👀 总播放量: {overview['views']['total']:,}")
...
# 同时更新 Agent 运行面板
import subprocess
agent_panel_script = os.path.join(os.path.dirname(__file__), 'update_agent_panel.py')
if os.path.exists(agent_panel_script):
    subprocess.run(['python3', agent_panel_script], check=False)
